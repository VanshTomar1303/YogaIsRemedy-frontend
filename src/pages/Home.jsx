...
export default function Home() {
    ...
    const handleSubmit = async () => {
        alert("Request Sent, wait a minute.");
        setIsLoading(true); // Set loading to true when the request starts
        try {
            const response = await ApiService.getRemedy(pida); // Pass `pida` to the API call
            setRemedy(response); // Set the retrieved remedy in state
            alert('Remedy retrieved successfully!');
        } catch (error) {
            console.error('Error retrieving data:', error);
            alert('An error occurred due to server issues.');
        } finally {
        ...
        <div className="bg-black min-h-[100vh] w-full">
            ...
            <div className="flex justify-center flex-col items-center p-4">
                <div className="flex justify-center items-center gap-5">
                    <input
                        className="h-[8vh] w-[40vw] p-4 border rounded-md border-red-700"
                        value={pida}
                        onChange={(e) => setPida(e.target.value)}
                        type="text"
                        placeholder="Enter your pida"
                    />
                    <button
                        onClick={handleSubmit} // No event parameter needed
                        className="text-2xl hover:border-white hover:border rounded-md p-4"
                    >
                        Search
                    </button>
                </div>
